...
API_KEY = '1a9b1b90e9207a9f2bc82d39e9bd5a6f'
...
def get_df(symbol, start_date, end_date):
    ...
    # filter dataframes by minimum date
    min_date = max(price_df.index.min(), balance_df.index.min())
    max_date = min(price_df.index.max(), balance_df.index.max())

    price_df = price_df[price_df.index >= min_date]
    balance_df = balance_df[balance_df.index >= min_date]

    price_df = price_df[price_df.index <= max_date]
    balance_df = balance_df[balance_df.index <= max_date] 

    corresponding = pd.DataFrame()
    for index, row in balance_df.iterrows():
        if index in price_df.index:
            new_row = price_df.loc[[index]]
        else:
            new_row = get_next_available(index, price_df)

        corresponding = pd.concat([corresponding, new_row])

    # concatenate DataFrames manually because Pandas is trash
    new_rows = []
    for (index1, row1), (index2, row2) in zip(balance_df.iterrows(), corresponding.iterrows()):
        newrow = pd.concat([row1, row2])
        new_rows.append(newrow)

    new_df = pd.DataFrame(new_rows)
    new_df.columns = list(balance_df.columns) + list(corresponding.columns)
